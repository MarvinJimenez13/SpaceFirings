...
public class BadShip2 {
    ...
    public static int aux=300;
    public boolean enters = true;
    private Sound shotShip, explo;
    private Preferencess prefs;

    public BadShip2(int x, int y, int binLefRig){
        this.x = x;
        this.y = y;
        prefs = new Preferencess();
        badTexture = new Texture("badship2.png");
        position = new Vector3(x,y,0);
        listBadBullet = new ArrayList<BadBullet>();
        bounds = new Rectangle(position.x, position.y,Gdx.graphics.getWidth()/2.77f, Gdx.graphics.getHeight()/5.92f);
    ...
    public void update(float dt){
        ...
        if(enters){
            ...
            if (binLefRig == 1) {
                x += mov;
                if (x > (Gdx.graphics.getWidth()- badTexture.getWidth())){
                    binLefRig = 0;
                }
            }
    ...
    public void shot(){
            if ((position.x % 50) == 0 || position.x <= 0 || position.x >= Gdx.graphics.getWidth()) {
                System.out.println("ABAJO DE 300 NAVE 22222222222222222222222222222");
                if (prefs.getSoudShipsBoo())
                    shotShip.play();
                listBadBullet.add(new BadBullet(position.x + ((badTexture.getWidth() / 2)), position.y + 10));
            }
